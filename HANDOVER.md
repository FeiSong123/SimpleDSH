...
- **Date:** 2026-08-11
- **Repo:** `/Users/oye/Downloads/projects/FlashCoder`, branch `main`
...
| `git log --oneline -10` | 8 commits; latest `02130f7` (see header); oldest `a1c34cb FlashCoder v0.1` |
...
- `release/flashcoder-0.1.0-rc.0.tgz` (268,555 bytes) exists; `npm run release` or the README curl→`npm install -g ./flashcoder.tgz` flow verifies it.
...
- Install quirk: npm refuses URL tarballs (`EALLOWREMOTE`), so releases must be downloaded to a file first, then `npm install -g ./flashcoder.tgz`. The package declares **no** install-time scripts, by design (supply-chain posture).
...
- `package.json`: name `flashcoder`, version `0.1.0-rc.0`, `private: true`, `license: UNLICENSED`, `type: module`, engines `node >=22`, bin `dist/src/cli.js`. Declares 3 direct runtime deps (`get-east-asian-width@1.6.0`, `marked@18.0.5`, `smol-toml@1.7.1`) + 2 devDeps (`typescript@6.0.3`, `@types/node@22.20.1`); `npm-shrinkwrap.json` pins them.
